...
\author[2]{Claude Opus 4.7\thanks{\texttt{Anthropic. Co-author for design + drafting; see \S\,Acknowledgements.}}}
\author[2]{Claude Opus 4.8\thanks{\texttt{Anthropic. Agent contributor: RRP-0029 protocol maintenance + paper-id model migration; see \S\,Acknowledgements.}}}
\affil[1]{Random Walks}
\affil[2]{Anthropic}
\date{\today}
...
\begin{claim}[Origin-agnostic OAuth]
...
\begin{claim}[Slugs are stable across revisions]
...
\begin{claim}[Author names are LaTeX-normalised]
...
\begin{claim}[Replication status is server-derived]
\label{claim:replication-status-server-derived}
A claim's \texttt{replication\_status} field is computed by the server from the accumulated annotation graph plus a per-discipline quorum (1 for formal verification, 2 for algorithms/crypto, 3 for ML and experimental sciences, 5 for behavioural/social), not read from the author-submitted CIR. A retraction annotation supersedes all other evidence; a contradiction with weight matching or exceeding supporting replications flips the status to \texttt{contradicted}; meeting the quorum of independent replications elevates it to \texttt{replicated}. Authors cannot self-certify replication.
\end{claim}

\begin{claim}[Snapshots are content-verifiable]
...
\begin{claim}[Annotation threads are artefact-rooted]
